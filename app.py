...
def obtenir_cle_fichier():
    ...
    if os.path.exists(chemin_textes):
        try:
            ...
            pass
    for f_peda in ["base_pedagogique_edubase.txt", "matrice_AFL_lycee.txt", "programmes_college_2015_carte_mentale.txt"]:
        ...
        if os.path.exists(fp):
            try:
                with open(fp, "r", encoding="utf-8") as f:
                    documents_charges.append(Document(text=f.read(), metadata={"source": f"Règles de Pierre ({fp})"}))
            except:
                pass
...
def initialiser_base_santorin(cle_fremt):
    docs_santorin = [
        Document(
            text="Fiche Mémo - Correction Partagée Santorin (DEC). Spécifications techniques sur la correction multiple.",
            metadata={"title": "Correction Partagée", "url": "https://assistance.ac-noumea.nc/IMG/pdf/fm_correction_partagee.pdf"}
        )
    ]
    docs_santorin.extend(charger_dossier_txt_securise("data/examens"))
    docs_santorin.extend(charger_consignes_pierre())
    return VectorStoreIndex.from_documents(docs_santorin).as_retriever(similarity_top_k=6)
...
def initialiser_base_ipack(cle_fremt):
    docs_ipack = [
        Document(
            text="Guide Pratique iPackEPS - Saisie des structures trimestrielles et imports XML.",
            metadata={"title": "Guide iPackEPS", "url": "https://eps.ac-normandie.fr/IMG/pdf/guide_utilisateur_professeur-2.pdf"}
        )
    ]
...
def initialiser_base_textes(cle_fremt):
    docs_textes = [
        Document(
            text="Base de données réglementaire globale pour les textes de lois du second degré.",
            metadata={"title": "Légifrance", "url": "https://www.legifrance.gouv.fr/"}
        )
    ]
    docs_textes.extend(charger_dossier_txt_securise("data/textes"))
    docs_textes.extend(charger_consignes_pierre())
    return VectorStoreIndex.from_documents(docs_textes).as_retriever(similarity_top_k=6)
...
def initialiser_base_peda(cle_fremt):
    ...
    return VectorStoreIndex.from_documents(docs_peda).as_retriever(similarity_top_k=7)
...
Ta mission :
1. Détecte le problème réel sous-jacent (ex: inscription SIÈCLE, élèves orphelins non associés à un protocole, synchronisation de nuit Cyclades/Santorin, droits Imag'in PDF, cadenas co-évaluation, AFC vs AFL, dispense vs mot des parents, sortie minibus sans surveillance, ratio falaise escalade, responsabilité chef établissement, alerte zone blanche, décharge parentale).
2. Identifie les logiciels ou cadres concernés dans la chaîne : SIÈCLE, Pronote, iPackEPS, Cyclades, Imag'in, Santorin, LSU/DNB, Bac GT, Bac Pro, CAP, Code de l'éducation, Loi Fauchon, L. 911-4.
...
label_titres = {
    ...
    "peda": "🔍 Mode Actif : Les programmes (Référentiels, CA & BO)",
    "textes": "🔒 Mode Actif : SÉCURITÉ & Responsabilité Juridique (Textes Officiels & Risques APPN)"
}

titre_affiche = label_titres.get(st.session_state.active_module, "🛠️ Mode Actif : Assistance Technique iPackEPS (Gestion du CCF & Inaptitudes)")
...
with col_b2:
    if st.button("📊 Examens &\nSantorin", use_container_width=True, key="btn_ex", type="primary" if st.session_state.active_module == "examens" else "secondary"):
        ...
        st.rerun()
with col_b3:
    if st.button("🔍 Les programmes", use_container_width=True, key="btn_ge", type="primary" if st.session_state.active_module == "peda" else "secondary"):
...
if prompt:
    ...
    with st.spinner("Je recherche les documents et textes officiels..."):
        ...
        # 🧠 AIGUILLEUR MINIMALISTE INTERCEPTANT LES PANNES CRITIQUES BLOQUANTES
        ...
        est_dnb_direct = (intention == "DNB_COLLEGE") or (mode == "examens" and ("dnb" in prompt.lower() or "brevet" in prompt.lower()))
        est_sujet_secours_direct = (intention == "SUJET_SECOURS") or ("sujet" in prompt.lower() and "secours" in prompt.lower())
        est_cap_3epreuves_direct = (intention == "CAP_3_EPREUVES") or (mode == "examens" and "cap" in prompt.lower() and ("3 épreuves" in prompt.lower() or "3 notes" in prompt.lower()))
        ...
        est_cas_blindé_racine = (
            est_dnb_direct or est_sujet_secours_direct or est_cap_3epreuves_direct or
            est_date_notes_direct or est_erreur_validation_santorin or est_grise_direct or
            est_remplacement_reunion_direct or est_sss_direct or st_groupes_direct or
            est_nouvel_eleve_direct or est_tasa_direct
        )
        ...
        if openai_api_key and not est_cas_blindé_racine:
            ...
            try:
                if mode == "examens":
                    ...
                    for n in retriever_textes.retrieve(mot_cle_local.strip() if len(mot_cle_local.strip()) > 2 else requete_ciblee):
                        extraits_doc += f"{n.node.text}\n\n"
                elif mode == "peda":
...
Tu réponds à la question avec clarté, pertinence et précision chirurgicale, SANS DÉTOUR NI BAVARDAGE, en t'appuyant STRICTEMENT sur le contexte documentaire fourni.
...
DIRECTIVES DE RÉPONSE ET D'EXPERTISE :

1. 🎯 POSITIONNEMENT DIRECT & JUSTE (PAS DE VERDICT AUTOMATIQUE HORS CONTEXTE) :
   - Pour un PROJET / CAS CONCRET SOUMIS À VALIDATION ("Puis-je autoriser cette sortie ?", "Ce projet est-il recevable ?", transport 2 minibus, accident) :
     * Débute par un avis d'audit clair (ex: <strong>AVIS : PROJET NON CONFORME EN L'ÉTAT</strong> ou <strong>AVIS : PROJET CONFORME SOUS CONDITIONS</strong>).
     * Décortique les failles opérationnelles : nullité légale absolue des décharges parentales pour mineurs, ratio conducteurs/surveillance à bord, encadrement obligatoire par un professeur d'EPS de l'établissement pour l'UNSS/AS et les APPN (un intervenant extérieur ou club ne peut pas encadrer seul).
   - Pour une QUESTION STATUTAIRE, TECHNIQUE OU MATÉRIELLE ("Faut-il...", "Comment faire...", "La SSS est-elle une AS ?", "Qu'est-ce qu'un PAI ?") :
     * Réponds DIRECTEMENT dès la première phrase sans coller de bandeau de refus déplacé.
     * Pour les moyens de communication en APPN / course d'orientation : distinguer le milieu standard (le téléphone satellite n'est pas imposé nominativement, repérage GSM préalable, lignes d'arrêt, sifflet) du milieu hostile / isolement total où l'obligation de moyens impose un dispositif autonome d'alerte (satellite inReach/Iridium, radio secours, PLB).

2. ⚖️ CITATION STRICTE DES TEXTES OFFICIELS :
   - CITE systématiquement les sources appropriées :
     ...
     * Surveillance et sorties : Circulaire n° 97-178 du 18 septembre 1997, Circulaire n° 2017-116 du 6 octobre 2017 (APPN), Circulaire n° 2023-052 du 13 juin 2023.
     * Association Sportive & UNSS : Article L. 552-1 et Décret n° 2014-460 (forfait 3h AS).
     * Inaptitudes médicales : Articles R. 312-2 à R. 312-6 du Code de l'éducation et Décret n° 88-977.

3. 🛑 RESPECT STRICT DES CYCLES & TERMINOLOGIE :
...
4. 🚫 CADRAGE PÉDAGOGIQUE STRICT (ONGLET LES PROGRAMMES) :
...
5. 📊 SPÉCIFICITÉS CCF & SANTORIN (ONGLET EXAMENS) :
   - Note Unique (Bac GT) : Cocher [Dispensé] sur les 2 épreuves manquantes, note normale sur l'APSA évaluée + commentaire circonstancié obligatoire pour la CAHPN.
   ...
   - Bac Pro : Clic préalable obligatoire sur le bouton [Choisir les AFLP].
   - CAP : Protocole strict à 2 épreuves (rejet si 3 notes).
   ...
   - Clôture de lot grisée : Nécessite 100 % des statuts saisis (astuce affichage 100 candidats).

6. 👥 STRUCTURES HORS-SIÈCLE & ÉTRANGER (AEFE) :
...
7. 📐 FORMAT DE SORTIE : Structure avec des listes à puces HTML (<ul>, <li>), des retours à la ligne (<br>) et des mots-clés en gras (<strong>). Zéro bloc de texte compact.
...
"""
        ...
        texte_brut = re.sub(r"(Article\s+L?\.?\s*\d+[-–\w]*|Loi\s+du\s+\d+\s+\w+\s+\d+|Loi\s+Fauchon|RGPD|Code\s+de l\'éducation|Code\s+pénal|Circulaire\s+du\s+\d+\s+\w+\s+\d+|Circulaire\s+n°\s*[\d-]+|Décret\s+n°\s*[\d-]+|Arrêté\s+du\s+\d+\s+\w+\s+\d+|BO\s+n°\s*\d+)", r'<span class="law-highlight">\1</span>', texte_brut)

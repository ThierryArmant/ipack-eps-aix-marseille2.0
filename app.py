...
if prompt:
    ...
    with st.spinner("Je recherche les documents..."):
        ...
        if est_tasa_direct:
            ...
            # ======================================================================
            # 🛠️ CORRECTION DU CRASH LIGNE 695 : CONSTRUCTION SÉCURISÉE DE CONSIGNE_IA
            # ======================================================================
            if mode == "examens":
                badge, color_card = "📊 EXAMENS & SANTORIN", "santorin-card"
            elif mode == "ipack":
            ...
            # Sécurité anti-hallucination si la boîte de contexte lue est vide pour les examens
            if mode == "examens" and not extraits_doc.strip():
                texte_brut = "Désolé, je ne trouve pas cette règle spécifique dans ma mémoire locale (`memoire_examens_santorin.txt`). Veuillez vérifier l'existence du fichier dans votre dossier data ou vous rapprocher de votre Chef d'établissement ou de votre IA-IPR EPS."
            else:
                consigne_ia = f"""Tu es l'assistant IA référent expert pour les enseignants d'EPS de l'académie d'Aix-Marseille.
        ...
        # SÉCURISATION DU RENDU HISTORIQUE (Remplacement des retours chariots)

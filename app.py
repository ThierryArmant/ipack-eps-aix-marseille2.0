...
def obtenir_cle_fichier():
    mtimes = []
    for fp in ["get_par_pierre.txt", "gere_par_pierre.txt", "data/examens/memoire_examens_santorin.txt", "data/textes/base_textes_officiels.txt"]:
...
# ======================================================================
# 7B. MESSAGES D'AVERTISSEMENT ET SYMÉTRIE VISUELLE
# ======================================================================
...
# ======================================================================
# 9. FLUX DE MESSAGES ET ARBITRAGE HYBRIDE (SÉCURISATION SOUVERAINE)
# ======================================================================
...
if prompt:
    ...
    with st.spinner("Je recherche les documents..."):
        ...
        # Récupération des règles de Pierre (Sûreté terrain)
        ...
        # 1. Classification sémantique de l'intention UNIQUEMENT pour sanctuariser iPack & Textes d'hier
        intention = "AUCUN_BLINDAGE"
        if mode == "ipack":
            intent_prompt = f"""Tu es l'aiguilleur du Hub IA-EPS pour l'onglet iPackEPS. Détermine l'INTENTION exacte.
            Question : "{prompt}"
            Réponds STRICTEMENT par un des mots-clés de cette liste restrictive :
            - IPACK_SSS : Problème de dossier SSS ou bilan annuel verrouillé par le chef d'établissement.
            - IPACK_GROUPES : Créer un groupe, configurer, associer un protocole/séquence d'APSA.
            - IPACK_NOUVEL_ELEVE : Ajouter un élève arrivant en cours d'année, synchronisation Pronote / SIECLE.
            - IPACK_TRANSFERT_DOUBLON : Élève muté avec des notes déjà acquises, ou fiche en doublon.
            - AUCUN_BLINDAGE : Autre question technique générale sur iPackEPS.
            """
            try: intention = Settings.llm.complete(intent_prompt).text.strip()
            except: intention = "AUCUN_BLINDAGE"
        elif mode == "textes":
            intent_prompt = f"""Tu es l'aiguilleur du Hub IA-EPS pour l'onglet Sécurité. Détermine l'INTENTION exacte.
            Question : "{prompt}"
            Réponds STRICTEMENT par un des mots-clés de cette liste restrictive :
            - SECURITE_TASA : La question concerne spécifiquement la taxe, la responsabilité liée au transport ou les déclarations TASA.
            - AUCUN_BLINDAGE : Autre question sur les textes juridiques généraux.
            """
            try: intention = Settings.llm.complete(intent_prompt).text.strip()
            except: intention = "AUCUN_BLINDAGE"
            if "tasa" in prompt.lower(): intention = "SECURITE_TASA"
        ...
        # ======================================================================
        # 2. APPLICATION DU BLINDAGE HISTORIQUE (SÉCURITÉ INTACTE POUR IPACK & TEXTES)
        # ======================================================================
        if mode == "ipack" and intention == "IPACK_SSS":
        ...
        elif mode == "ipack" and intention == "IPACK_GROUPES":
        ...
        elif mode == "ipack" and intention == "IPACK_NOUVEL_ELEVE":
        ...
        elif mode == "ipack" and intention == "IPACK_TRANSFERT_DOUBLON":
        ...
        elif mode == "textes" and intention == "SECURITE_TASA":
            try:
                for n in retriever_textes.retrieve(prompt): extraits_doc += f"{n.node.text}\n\n"
            except: pass
            texte_brut = extraits_doc if extraits_doc else "Aucun extrait TASA identifié."
            badge, color_card = "⚖️ TEXTES OFFICIELS", "securite-card"

        # ======================================================================
        # 3. MOTEUR RAG DYNAMIQUE (EXAMENS PUR + RÉPARATIONS DES AUTRES ONGLETS)
        # ======================================================================
        else:
            if mode == "examens":
                badge, color_card = "📊 EXAMENS & SANTORIN", "santorin-card"
                retriever_cible = retriever_santorin
            elif mode == "ipack":
                badge, color_card = "🛠️ ASSISTANCE iPACKEPS", "general-card"
                retriever_cible = retriever_ipack
            elif mode == "textes":
                badge, color_card = "⚖️ SÉCURITÉ & CADRE JURIDIQUE", "securite-card"
                retriever_cible = retriever_textes
            else:
                badge, color_card = "🔍 CADRAGE & RÉFÉRENTIELS BO", "peda-card"
                retriever_cible = retriever_peda

            try:
                for n in retriever_cible.retrieve(prompt): extraits_doc += f"{n.node.text}\n\n"
            except: pass

            consigne_ia = f"""Tu es l'assistant IA référent du Hub EPS de l'académie d'Aix-Marseille.
            Réponds à la question de l'enseignant de façon structurée et chirurgicale en exploitant le contexte ci-dessous.
            
            CONTEXTE APPLICABLE RECHERCHÉ :
            {extraits_doc}
            {verites_terrain_pierre}
            ...
            - Pour l'onglet Examens, fie-toi exclusivement au contenu : discrimine rigoureusement les filières (Bac GT, Bac Pro, CAP, DNB/Collège).
            - Présente ta réponse avec des listes à puces HTML (<ul>, <li>), des sauts de lignes et des textes en gras (<strong>) pour isoler les étapes. Ne génère pas de balises <html> globales.
            - Si les documents ne mentionnent pas d'élément de réponse stable, invite l'utilisateur à consulter la direction de son établissement ou son IA-IPR EPS.
            """
            response = Settings.llm.complete(consigne_ia)
            texte_brut = response.text

        # ======================================================================
        # 4. FILTRES DE RENDU ET ENRICHISSEMENTS (MISES EN FORME COMMUNES)
        # ======================================================================

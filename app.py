...
# --- FONCTION DE CHARGEMENT : PIERRE_DATA.PY ---
@st.cache_resource
def charger_consignes_pierre():
    chemin_fichier = "pierre_data.py"
    if os.path.exists(chemin_fichier):
        try:
            docs = SimpleDirectoryReader(input_files=[chemin_fichier]).load_data()
            st.sidebar.success(f"✅ Fichier '{chemin_fichier}' chargé.")
            return docs
        except Exception as e:
            st.sidebar.error(f"❌ Erreur lecture : {e}")
            return []
    else:
        st.sidebar.warning(f"⚠️ Fichier '{chemin_fichier}' introuvable.")
        return []
...
st.markdown(f"""
    ...
    .santorin-card, .general-card, .securite-card {{ background-color: rgba(15, 23, 42, 0.45) !important; backdrop-filter: blur(12px) !important; -webkit-backdrop-filter: blur(12px) !important; padding: 18px; border-radius: 8px; margin-bottom: 16px; box-shadow: 0px 6px 20px rgba(0,0,0,0.5); }}
    ...
    .santorin-card a, .general-card a, .securite-card a, .santorin-card a *, .general-card a *, .securite-card a * {{ color: #FFB020 !important; text-decoration: underline !important; font-weight: 600 !important; text-shadow: 1px 1px 2px rgba(0,0,0,0.9) !important; }}
...
def initialiser_base_santorin():
    docs_santorin = [
        ...
        # ... [RESTE DE TES DOCUMENTS ICI] ...
    ]
    docs_santorin.extend(charger_consignes_pierre()) # <-- INTÉGRATION PIERRE_DATA.PY
...
def initialiser_base_ipack():
    ...
    docs_ipack.extend(charger_consignes_pierre()) # <-- INTÉGRATION PIERRE_DATA.PY
...
# --- (RESTE DU CODE : BANDEAU, BOUTONS, LOGIQUE IA) ---
# [COLLE ICI TOUT LE RESTE DE TON CODE ORIGINAL À PARTIR DE LA SECTION 5]
